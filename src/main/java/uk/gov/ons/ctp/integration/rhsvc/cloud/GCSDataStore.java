...
public class GCSDataStore implements CloudDataStore {
   ...
   */
  @Override
  public void storeObject(final String bucket, final String key, final String value) throws StorageException {
   ...
   */
  @Override
  public Optional<String> retrieveObject(final String bucket, final String key) throws StorageException {
    Storage storage = StorageOptions.getDefaultInstance().getService();
  ...
  private void storeObject(String bucket, String key, String value, Storage storage) throws StorageException {
    BlobId blobId = BlobId.of(bucket, key);
  ...
  private boolean retrieveObject(String bucket, String key, Blob blob) {
    if (null == blob) {
      log.with(key).debug("Object could not be retrieved within cloud in bucket = <" + bucket + ">");
      return true;
    }
    return false;
  }
