pipeline {
    ...
    stages {
        ...
        stage('dev') {
            ...
            steps {
                sh "find . -type f -name '*rhsvc*.jar' -not -name '*docker-info*' -exec mv {} target/rhsvc.jar \\;"
        ...
        stage('test') {
            ...
            steps {
                ...
                sh 'sh artifactory-get.sh -r http://artifactory-sdc.onsdigital.uk/artifactory/libs-release-local/ -g uk.gov.ons.ctp.product -a rhsvc > target/rhsvc.jar'
                sh 'chmod 777 target/rhsvc.jar'
